...
pub static SYSTEM_TABLE: tpl_mutex::TplMutex<Option<EfiSystemTableOld>> =
    tpl_mutex::TplMutex::new(efi::TPL_NOTIFY, None, "StLock");

/// EFI Runtime Services Table.
///
/// This implementation assumes that the Runtime Services table is externally observable outside Patina, and ensures
/// all data accesses to the table are done via volatile reads/writes to ensure that they are not elided or reordered.
pub struct EfiRuntimeServicesTable {
    runtime_services: *mut efi::RuntimeServices,
}

impl EfiRuntimeServicesTable {
    /// Allocates a new Runtime Services table initialized to default stub functions in the given allocator.
    pub fn allocate_new_table(allocator: &dyn Allocator) -> Self {
        let rt = Self::default_runtime_services_table();

        let (runtime_services, _) = Box::into_raw_with_allocator(Box::new_in(rt, allocator));
        let mut table = Self { runtime_services };
        table.checksum();
        table
    }

    /// Creates a new EfiRuntimeServicesTable instance from the given raw pointer.
    ///
    /// # Safety
    /// The caller must ensure that the provided pointer is valid and points to a properly initialized Runtime Services table.
    pub unsafe fn from_raw_ptr(ptr: *mut efi::RuntimeServices) -> Self {
        Self { runtime_services: ptr }
    }

    // Checksums the Runtime Services table. Assumes the table is externally observable outside Patina.
    fn checksum(&mut self) {
        // SAFETY: EfiRuntimeServicesTable construction ensures that the pointer is valid.
        let mut table_copy = unsafe { self.runtime_services.read_volatile() };
        table_copy.hdr.crc32 = 0;

        let tbl_ptr = &raw const table_copy as *const u8;
        // SAFETY: table is a local copy so data and alignment are correct. efi::RuntimeServices is a `repr(C)` struct.
        let tbl_slice = unsafe { from_raw_parts(tbl_ptr, size_of::<efi::RuntimeServices>()) };
        table_copy.hdr.crc32 = crc32fast::hash(tbl_slice);

        // SAFETY: EfiRuntimeServicesTable construction ensures that the pointer is valid.
        unsafe {
            self.runtime_services.write_volatile(table_copy);
        }
    }
    ...
    pub fn get(&self) -> efi::RuntimeServices {
        // SAFETY: EfiRuntimeServicesTable construction ensures that the pointer is valid.
        unsafe { self.runtime_services.read_volatile() }
    }

    /// Writes copies the given Runtime Services table instance into the stored table and updates the checksum.
    pub fn set(&mut self, new_table: efi::RuntimeServices) {
        // SAFETY: EfiRuntimeServicesTable construction ensures that the pointer is valid.
        unsafe {
            self.runtime_services.write_volatile(new_table);
        }
        self.checksum();
    }

    /// Returns the inner raw pointer to the Runtime Services table.
    pub fn as_mut_ptr(&mut self) -> *mut efi::RuntimeServices {
...
///
/// This implementation assumes that the Boot Services table is externally observable outside Patina, and ensures
/// all data accesses to the table are done via volatile reads/writes to ensure that they are not elided or reordered.
...
impl EfiBootServicesTable {
    /// Allocates a new Runtime Services table initialized to default stub functions in the given allocator.
    pub fn allocate_new_table(allocator: &dyn Allocator) -> Self {
        let rt = Self::default_boot_services_table();

        let (boot_services, _) = Box::into_raw_with_allocator(Box::new_in(rt, allocator));
        let mut table = Self { boot_services };
        table.checksum();
        table
    }

    /// Creates a new EfiBootServicesTable instance from the given raw pointer.
    ///
    /// # Safety
    /// The caller must ensure that the provided pointer is valid and points to a properly initialized Boot Services table.
    pub unsafe fn from_raw_ptr(ptr: *mut efi::BootServices) -> Self {
        Self { boot_services: ptr }
    }

    // Checksums the Boot Services table. Assumes the table is externally observable outside Patina.
    fn checksum(&mut self) {
        // SAFETY: EfiBootServicesTable construction ensures that the pointer is valid.
        let mut table_copy = unsafe { self.boot_services.read_volatile() };
        table_copy.hdr.crc32 = 0;

        let tbl_ptr = &raw const table_copy as *const u8;
        // SAFETY: table is a local copy so data and alignment are correct. efi::BootServices is a `repr(C)` struct.
        let tbl_slice = unsafe { from_raw_parts(tbl_ptr, size_of::<efi::BootServices>()) };
        table_copy.hdr.crc32 = crc32fast::hash(tbl_slice);

        // SAFETY: EfiBootServicesTable construction ensures that the pointer is valid.
        unsafe {
            self.boot_services.write_volatile(table_copy);
        }
    }
    ...
    pub fn get(&self) -> efi::BootServices {
        // SAFETY: EfiBootServicesTable construction ensures that the pointer is valid.
        unsafe { self.boot_services.read_volatile() }
    }

    /// Writes copies the given Boot Services table instance into the stored table and updates the checksum.
    pub fn set(&mut self, new_table: efi::BootServices) {
        // SAFETY: EfiBootServicesTable construction ensures that the pointer is valid.
        unsafe {
            self.boot_services.write_volatile(new_table);
        }
        self.checksum();
    }

    /// Returns the inner raw pointer to the Boot Services table.
    pub fn as_mut_ptr(&mut self) -> *mut efi::BootServices {
        self.boot_services
    }
...
impl EfiSystemTable {
    /// Allocates a new EFI System Table initialized to default contents.
    /// Also creates default Boot Services and Runtime Services tables.
    pub fn allocate_new_table(allocator: &dyn Allocator) -> Self {
        let st = Self::default_system_table();

        let (system_table, _) = Box::into_raw_with_allocator(Box::new_in(st, allocator));
        let mut table = Self { system_table };
        table.checksum();
        table
    }

    /// Creates a new EfiSystemTable instance from the given raw pointer.
    ///
    /// # Safety
    /// The caller must ensure that the provided pointer is valid and points to a properly initialized System Table.
    pub unsafe fn from_raw_ptr(ptr: *mut efi::SystemTable) -> Self {
        Self { system_table: ptr }
    }

    // Checksums the System Table. Assumes the table is externally observable outside Patina.
    fn checksum(&mut self) {
        // SAFETY: EfiSystemTable construction ensures that the pointer is valid.
        let mut table_copy = unsafe { self.system_table.read_volatile() };
        table_copy.hdr.crc32 = 0;

        let tbl_ptr = &raw const table_copy as *const u8;
        // SAFETY: table is a local copy so data and alignment are correct. efi::SystemTable is a `repr(C)` struct.
        let tbl_slice = unsafe { from_raw_parts(tbl_ptr, size_of::<efi::SystemTable>()) };
        table_copy.hdr.crc32 = crc32fast::hash(tbl_slice);

        // SAFETY: EfiSystemTable construction ensures that the pointer is valid.
        unsafe {
            self.system_table.write_volatile(table_copy);
        }
    }
    ...
    pub fn get(&self) -> efi::SystemTable {
        // SAFETY: EfiSystemTable construction ensures that the pointer is valid.
        unsafe { self.system_table.read_volatile() }
    }

    /// Writes copies the given System Table instance into the stored table and updates the checksum.
    pub fn set(&mut self, new_table: efi::SystemTable) {
        // SAFETY: EfiSystemTable construction ensures that the pointer is valid.
        unsafe {
            self.system_table.write_volatile(new_table);
        }
        self.checksum();
    }

    /// Returns the inner raw pointer to the System Table.
    pub fn as_mut_ptr(&mut self) -> *mut efi::SystemTable {
        self.system_table
    }
...
/* pub struct EfiSystemTableOld {
    system_table: Box<efi::SystemTable, &'static dyn Allocator>,
    boot_service: EfiBootServicesTableOld, // These fields ensure the efi::BootServices and efi::RuntimeServices structure pointers (in
    runtime_service: EfiRuntimeServicesTableOld, // the system_table) have the same lifetime as the EfiSystemTable.
}

impl EfiSystemTableOld {
    fn init() -> EfiSystemTableOld {
        ...
        let mut bs = EfiBootServicesTableOld::init();
        let mut rt = EfiRuntimeServicesTableOld::init();
...
impl AsMut<efi::SystemTable> for EfiSystemTableOld {
    fn as_mut(&mut self) -> &mut efi::SystemTable {
        self.system_table.as_mut()
    }
}

impl AsRef<efi::SystemTable> for EfiSystemTableOld {
...
unsafe impl Sync for EfiSystemTableOld {}
unsafe impl Send for EfiSystemTableOld {} */

pub fn init_system_table() {
    let mut table = EfiSystemTableOld::init();
    table.checksum();
