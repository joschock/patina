...
use patina::{
    guids,
    pi::{error_codes::EFI_NOT_AVAILABLE_YET, protocols, status_code},
};
...
struct ArchProtocolPointer<T> {
    proto: Once<*mut T>,
}
// Safety: ArchProtocolPointer uses Once which is Sync, and T is only accessed through immutable references after initialization.
unsafe impl<T> Sync for ArchProtocolPointer<T> {}

impl<T> ArchProtocolPointer<T> {
    const fn new() -> Self {
        ArchProtocolPointer { proto: Once::new() }
    }
    // Safety: Caller must ensure that ptr is a valid (non-null)pointer to T and that init is only called once.
    unsafe fn init(&self, ptr: *mut T) {
        assert!(!self.proto.is_completed());
        assert!(!ptr.is_null());
        self.proto.call_once(|| ptr);
    }

    fn get(&self) -> Option<(&T, *mut T)> {
        if let Some(ptr) = self.proto.get() {
            let ptr = *ptr;
            // Safety: init() ensures that ptr is valid and non-null.
            unsafe { Some((&*ptr, ptr)) }
        } else {
            None
        }
...
static METRONOME_ARCH_PTR: ArchProtocolPointer<protocols::metronome::Protocol> = ArchProtocolPointer::new();
static WATCHDOG_ARCH_PTR: ArchProtocolPointer<protocols::watchdog::Protocol> = ArchProtocolPointer::new();
...
extern "efiapi" fn stall(microseconds: usize) -> efi::Status {
    if let Some((metronome, metronome_ptr)) = METRONOME_ARCH_PTR.get() {
...
extern "efiapi" fn set_watchdog_timer(
    ...
    if let Some((watchdog, watchdog_ptr)) = WATCHDOG_ARCH_PTR.get() {
...
extern "efiapi" fn metronome_arch_available(event: efi::Event, _context: *mut c_void) {
    match PROTOCOL_DB.locate_protocol(protocols::metronome::PROTOCOL_GUID) {
        Ok(metronome_arch_ptr) => {
            unsafe {
                METRONOME_ARCH_PTR.init(metronome_arch_ptr as *mut protocols::metronome::Protocol);
            }
...
extern "efiapi" fn watchdog_arch_available(event: efi::Event, _context: *mut c_void) {
    match PROTOCOL_DB.locate_protocol(protocols::watchdog::PROTOCOL_GUID) {
        Ok(watchdog_arch_ptr) => {
            unsafe {
                WATCHDOG_ARCH_PTR.init(watchdog_arch_ptr as *mut protocols::watchdog::Protocol);
            }
            if let Err(status_err) = EVENT_DB.close_event(event) {
